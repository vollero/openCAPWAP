...
CWBool nl80211CmdSetStation(WTPBSSInfo * infoBSS, WTPSTAInfo staInfo){
	...
	CWLog("NL80211_CMD_SET_STATION. WLanID: %d, MacAddr[0](%02x) - MacAddr[4](%02x)", infoBSS->interfaceInfo->realWlanID, (int)staInfo.address[0], (int)staInfo.address[4]);
...
int CWInjectFrameMonitor2(int rawSocket, void *data, size_t len, int encrypt, int noack)
{
	...
	if (ppcap == NULL) {
		fprintf(stderr,"Unable to open interface in pcap: %s\n", szErrbuf);
		return (1);
	}
	...
	switch (nLinkEncap) {
		...
		default:
			fprintf(stderr,"!!! unknown encapsulation on interface !\n");
			return (1);
	...
	if (pcap_compile(ppcap, &bpfprogram, szProgram, 1, 0) == -1) {
		...
		if (pcap_setfilter(ppcap, &bpfprogram) == -1) {
			...
			fprintf(stderr,"RX Filter applied\n");
		}
	...
	fprintf(stderr,"   (delay between packets %dus)\n", nDelay);
	...
	if(!fBrokenSocket) {
		...
		if (ieee80211_radiotap_iterator_init(&rti,
		    (struct ieee80211_radiotap_header_out *)pu8Payload,
		    bytes) < 0)
			return 0; //continue;
		...
		fprintf(stderr,"RX: Rate: %2d.%dMbps, Freq: %d.%dGHz, "
		    "Ant: %d, Flags: 0x%X\n",
